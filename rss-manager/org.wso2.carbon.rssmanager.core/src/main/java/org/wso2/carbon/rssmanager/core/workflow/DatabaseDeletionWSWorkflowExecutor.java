...
public class DatabaseDeletionWSWorkflowExecutor extends WorkflowExecutor {

	private static final Log log = LogFactory.getLog(DatabaseDeletionWSWorkflowExecutor.class);

	@XmlRootElement(name = "DropDBApprovalWorkFlowProcessRequest")
	@XmlAccessorType(XmlAccessType.FIELD)
	static class DropDBApprovalWorkFlowProcessRequest extends WFMessage {

	}

	WorkflowManager wm;

	public DatabaseDeletionWSWorkflowExecutor() {
	...
	public void execute(Workflow workflow) throws WorkflowException {
		if (log.isDebugEnabled()) {
			log.info("Executing Drop database Workflow");
		}
	...
	public void buildSOAPMessage(Workflow wflow) {
		...
		DropDBApprovalWorkFlowProcessRequest ddbWf = new DropDBApprovalWorkFlowProcessRequest();
		ddbWf.setEnvironment(wflow.getType());
		ddbWf.setCallbackURL(wflow.getCallbackURL());
		ddbWf.setParameters(wflow.getParameterList());

		JAXBContext context;
		try {
			context = JAXBContext.newInstance(DropDBApprovalWorkFlowProcessRequest.class);
